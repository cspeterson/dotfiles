...
ssh_mux() {
  ...
  if [ "${1}" == 'tmux' ]; then
    ssh -t ${2} "clear; echo \"Logging into host ${2}  identifying as ${hostnameshellcmd}\"; ${sudocmd} tmux -f ~$(whoami)/.tmux.conf attach-session -t ${session_name} || ${sudocmd} tmux -f ~$(whoami)/.tmux.conf new-session -s ${session_name}"
  else
